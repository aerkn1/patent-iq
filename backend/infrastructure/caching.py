...
class CacheManager:
    ...
    def ensure_cache(cls, registry: Dict[str, str]) -> Dict[str, str]:
        ...
        logger.info(f"Checking data cache for {total_files} files (Parallel)...")
            ...
            try:
                ...
                if not local_path.exists():
                    ...
                    if remote_etag and local_etag != remote_etag:
                        needs_download = True
                        logger.info(f"Update available: {filename}")

            except Exception as e:
                logger.warning(f"Could not check remote status for {filename}: {e}")
        ...
        logger.info("Data cache verification complete.")
